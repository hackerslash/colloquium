...
**Action:** When working on this application, always review available global shortcuts ( and  state) to see if there are useful application modes that lack clear visual entry points, and surface them via standard  components.
## $(date +%Y-%m-%d) - Discovered Hidden Functionality
**Learning:** The application had a search feature that was primarily accessible through keyboard shortcuts or hidden entry points. Exposing this via a dedicated icon button significantly improves discoverability and mouse/touch usability.
